...
public class BasicInvertedIndex extends Configured implements Tool
{
            ...
            Mapper<Object, Text, Text, Text>
    {
                ...
                throws IOException, InterruptedException
        {
        	...
        	// Each TOKEN is associated with its source file 
        	while (itr.hasMoreTokens())
        	{
        		String token = itr.nextToken().toLowerCase().replaceAll("[^A-Za-z]", ""); 
        		TOKEN.set(token); context.write(TOKEN, INPUTFILE);
        	}
    ...
    public static class Reduce extends Reducer<Text, Text, Text, ArrayListWritable<Text>>
    {
        ...
        {
        	ArrayListWritable<Text> indexvalue = new ArrayListWritable<Text>();
        	
        	//For each of the values in the input, place them in an output
        	//array, in sorted order

        	Iterator<Text> iter = values.iterator();

        	while (iter.hasNext()) {
        		//We need a new object for adding to the collection 
        		Text fileNameText = new Text(iter.next());

        		//Remove duplicates - optional for Exercise1.2

        		//Would be more efficient if collection ordered 
        		String fileName = fileNameText.toString(); Iterator<Text> indexiter = indexvalue.iterator(); 

        		boolean found = false;

        		while (indexiter.hasNext() && !found) {
        			if (fileName.compareTo(indexiter.next().toString()) == 0) found = true;
        		}

        		if (!found) 
        			indexvalue.add(fileNameText);
        	}

        	Collections.sort(indexvalue);
